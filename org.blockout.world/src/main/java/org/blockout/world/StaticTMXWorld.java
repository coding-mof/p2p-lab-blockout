...
public class StaticTMXWorld implements IWorld {
	...
	public Tile getTile( final int x, final int y ) {
		...
		if ( objectId == 586 ) {
			return new Tile( spriteId, new Zombie( 5 ) ); // SpriteType.Crate
		}
		if ( objectId == 249 ) {
			return new Tile( spriteId, new Zombie( 5 ) ); // SpriteType.Zombie
		}
		if ( objectId == 250 ) {
			return new Tile( spriteId, new Monster( "Skeleton", 5, 100, 25 ) ); // SpriteType.Skeleton
		}
