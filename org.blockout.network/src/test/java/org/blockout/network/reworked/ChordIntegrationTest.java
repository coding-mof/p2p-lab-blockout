...
public class ChordIntegrationTest {
	...
	private IChordOverlay createAndConnectPeer( final LocalNode node, final ChordListener listener ) {
		...
		overlay = new ChordOverlayChannelHandler( node, scheduler, new SyncTaskExecutor(), 30000, 1000, 2000 );
	...
	public void setUp() {
		...
		ThreadPoolTaskScheduler scheduler = createPooledScheduler();
		chord = new ChordOverlayChannelHandler( localNode, scheduler, new SyncTaskExecutor(), 30000, 1000, 2000 );
