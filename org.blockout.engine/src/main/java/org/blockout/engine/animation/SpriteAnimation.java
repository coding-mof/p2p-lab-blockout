...
public class SpriteAnimation implements IAnimation {
     ...
     *             If the spritesheet was not loaded or there are no frames or
     *             the duration was negative
     */
    public SpriteAnimation( ISpriteSheet spriteSheet, int[] frameIds,
            int duration ) {
        Preconditions.checkNotNull( spriteSheet );
        Preconditions.checkArgument( spriteSheet.isSpriteSheetLoaded(),
                "SpriteSheet has an invalid state" );
        Preconditions.checkArgument( 0 < frameIds.length,
        ...
        for ( int id : frameIds ) {
            Image img = spriteSheet.getSprite( id );
            if( null != img )
                images.add( img );
        }

        animation = new Animation( (Image[]) images.toArray( new Image[0] ),
                duration );
    }
